...
* #28: Added five example workflows under `examples/` covering all nine operations — a parameterized-query report, an idempotent mini-ETL with upsert, a full insert/update/delete lifecycle, a schema-explorer data dictionary, and an AI-agent workflow using the node as a tool — plus an `examples/README.md` describing import steps, expected output, and the safety design of each. The write workflows resolve their target schema at runtime via `SELECT COALESCE(CURRENT_SCHEMA, CURRENT_USER)` instead of hardcoding one, so they run unchanged against any user's credential.
